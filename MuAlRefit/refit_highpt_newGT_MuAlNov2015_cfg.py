...
process.GlobalTag.globaltag = "74X_dataRun2_Candidate_2015_11_18_10_21_42"
...
process.muonDtAlignment = cms.ESSource("PoolDBESSource", CondDBSetup,
                                     ...
                                     toGet   = cms.VPSet(cms.PSet(record = cms.string("DTAlignmentRcd"),  tag = cms.string("DTAlignment_Nov2015_7412p4_data-Run2015D-Prompt-v3_TB110001_v1")))
                                     )
...
process.muonCscAlignment = cms.ESSource("PoolDBESSource", CondDBSetup,
                                     connect = cms.string('frontier://FrontierProd/CMS_CONDITIONS'),
                                     toGet   = cms.VPSet(cms.PSet(record = cms.string("CSCAlignmentRcd"), tag = cms.string("CSCAlignment_Nov2015_7412p4_data-Run2015D-Prompt-v3_RingsMoveGlobal-TB100001_v1")))
                                     )
...
process.globalPosition = cms.ESSource("PoolDBESSource", CondDBSetup,
                                     connect = cms.string('frontier://FrontierProd/CMS_CONDITIONS'),
                                     toGet   = cms.VPSet(cms.PSet(record = cms.string("GlobalPositionRcd"), tag = cms.string("GlobalPosition_MuonGPR_Nov2015_74x_data-Run2015D-Prompt-v3v4_v1")))
                                     )
process.es_prefer_globalPosition = cms.ESPrefer("PoolDBESSource","globalPosition")

process.muonPtFilter = cms.EDFilter("MuonPtFilter")

process.Path = cms.Path(process.muonPtFilter * process.MeasurementTrackerEvent * process.muAlGeneralTracks * process.muAlAncientMuonSeed * process.muAlStandAloneMuons * process.muAlGlobalMuons * process.muAlTevMuons * process.muAlGlbTrackQual * process.muAlMuons)
